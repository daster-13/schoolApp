...
//    }
    ...
    @PostMapping("/insert")
    public String teacherInsert(@Valid @ModelAttribute("teacherInsertDTO") TeacherInsertDTO teacherinsertDTO, BindingResult bindingResult, Model model,
        ...
        try {
            //save teacher to DB
            //      TeacherReadOnlyDTO teacherReadOnlyDTO = new TeacherReadOnlyDTO("acf-1234", "Αθανάσιος", "Ανδρούτσος");
    ...
    @GetMapping({"", "/"})
    public String getPaginatedTeachers(@PageableDefault(page = 0, size = 5, sort = "lastname") Pageable pageable, Model model) {
        Page<TeacherReadOnlyDTO> teacherPage = teacherService.getPaginatedTeachersDeletedFalse(pageable);
//        Page<TeacherReadOnlyDTO> teacherPage = teacherService.getPaginatedTeachers(pageable);
...
//                );
    ...
    public String getTeacherEdit(@PathVariable UUID uuid, Model model) {
        try {
            ...
            TeacherEditDTO teacherEditDTO = teacherService.getTeacherByUUIDAndDeletedFalse(uuid);
    ...
    public String deletedTeacher(@PathVariable UUID uuid, Model model, RedirectAttributes redirectAttributes) {
        try {
            TeacherReadOnlyDTO readOnlyDTO = teacherService.deletedTeacherByUUID(uuid);
            redirectAttributes.addFlashAttribute("teacherReadOnlyDTO", readOnlyDTO);
    ...
    @ModelAttribute("regionReadOnlyDTO") // Εκτελέιται πριν από κάθε request handler
