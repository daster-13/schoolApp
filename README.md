...
- Ports: Default server port is 8080 (can be changed via server.port)
...
- staging — placeholder (minimal properties).
...
MySQL sample commands (in MySQL shell):

CREATE DATABASE school9ssr CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'user9'@'%' IDENTIFIED BY '12345';
GRANT ALL PRIVILEGES ON school9ssr.* TO 'user9'@'%';
FLUSH PRIVILEGES;
...
Note: The resulting artifact is under build/libs (example: schoolapp-0.0.1-SNAPSHOT.jar).

Run the JAR:

Windows: java -jar build\libs\schoolapp-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod
macOS/Linux: java -jar build/libs/schoolapp-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod
...
Current migration files (if present):
